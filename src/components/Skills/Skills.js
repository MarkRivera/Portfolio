...
import { faHtml5, faReact, faCss3, faSass, faBootstrap, faJs, faNpm } from '@fortawesome/free-brands-svg-icons';
			...
			<section className="skill-container">
...

				{
					this.state.skills.map((element, index) => {
						return (
							<div key={`${element.skill+ ' ' +[index]}`} className=" skill-box">
								<div className={element.tech} key={`${element.tech + ' ' + index}`}> 
										...
										element.skill.map((item, index) => {
											return (
												<div className="tech" key={`${item +' '+ index}`}>
													<FontAwesomeIcon icon={element.icons[index]} size="6x" className={ `skill-icons ${item}` } />
